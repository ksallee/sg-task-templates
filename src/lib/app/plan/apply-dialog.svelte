<!--
	Apply's confirm: one line of totals, then only the risky items (apply-confirm.ts), the undo
	warning when the store is download-only, and Confirm / Cancel. Confirm is the caller's.
-->
<script lang="ts">
...
<Dialog.Root bind:open>
	<Dialog.Content class="sm:max-w-xl" data-slot="apply-dialog">
		...
		{#if content.sections.length || !persistent}
		...
		<Dialog.Footer>
			...
			<Button onclick={onConfirm} data-slot="apply-confirm">Confirm</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
