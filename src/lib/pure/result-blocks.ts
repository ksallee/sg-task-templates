...
const nameOf = (t: { content: string | null }) =>
  (t.content ?? "").trim() || "(no name)";
...
export function entityTasks(
  ...
  const add = (
    ...
    line: Omit<ResultTaskLine, "label">,
  ) => {
  ...
  for (const row of plan.rows) {
    if (row.kind === "keep" || row.kind === "claim") {
      const note = row.rename
        ? `Renamed from ${nameOf({ content: row.rename.from })}`
        : null;
...
export function resultBlocks(
  ...
  return [...rows]
    ...
    .map(({ r }) => {
      ...
      const applied =
        (r.kind === "clean" || r.kind === "differences") &&
        plan &&
        result?.kind === "ok";
      return {
        ...r,
        steps: applied
          ? entityTasks(plan, result.createdFor ?? [], omitStatus)
          : [],
...
export function runTotals(blocks: ResultBlock[]): RunTotal[] {
  ...
  return TOTAL_ORDER.map((kind) => {
    const count = lines.filter(
      (l) => l.kind === kind && (kind !== "create" || l.taskId !== null),
    ).length;
