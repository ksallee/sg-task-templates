...
const tt = (
  ...
  content: string,
  s: EntityRef | null,
): TemplateTask => ({ ...core(id, content, s), templateId: 5 });
const task = (
  id: number,
  content: string,
  s: EntityRef | null,
...
describe("entityTasks", () => {
  it("groups the Tasks by Pipeline Step in plan order, one line each with its outcome", () => {
    const groups = entityTasks(
      shot1,
      [{ templateTaskId: 502, taskId: 900 }],
      "omt",
    );
    expect(
      groups.map((g) => [
        g.step,
        g.lines.map((l) => [l.name, l.taskId, l.label, l.note]),
      ]),
    ).toEqual([
      [
        "Lighting",
...
describe("resultBlocks", () => {
  it("gives each applied entity its Tasks; a failed or undone one none; a stored one its record tally", () => {
    const rows = [
      row(1, "clean"),
      row(2, "failed", { error: "Bad", record: null }),
      row(3, "undone"),
      row(4, "landed"),
    ];
    const outcomes = [
  ...
  it("shows what needs a look first: failed, then with differences, then the rest in run order", () => {
    ...
    expect(resultBlocks(rows, [], [], "omt").map((b) => b.label)).toEqual([
      "sh3",
      "sh2",
      "sh1",
      "sh4",
...
describe("runTotals", () => {
  it("adds the Tasks of the applied entities by outcome, in the plan’s words", () => {
    ...
    const blocks = resultBlocks(
      [row(1, "clean"), row(2, "failed")],
      [shot1, plan(2, shot1.rows)],
      outcomes,
      "omt",
    );
