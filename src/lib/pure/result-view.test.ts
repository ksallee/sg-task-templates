...
const tt = (id: number, content: string): TemplateTask => ({
  ...core(id, content),
  templateId: 5,
});
...
const plan: EntityPlan = {
  ...
  rows: [
    ...
    {
      kind: "create",
      templateTask: tt(501, "Grade"),
      templateDates: { start: null, due: null },
      datesClearable: false,
    },
  ],
  edges: {
    expectedAdded: [],
      ...
      describeDifference(
        {
          ...
          expected: [{ type: "HumanUser", id: 3, name: "Ann" }],
          actual: [],
        },
        names,
      ),
...
describe("describeDifference", () => {
  ...
  it("reads each difference as a sentence", () => {
    expect(
      describeDifference(
        { code: "create_missing", templateTaskId: 501 },
        names,
      ),
    ).toBe("Grade was not created.");
    expect(
      describeDifference(
        { code: "create_unexpected", taskId: 42, templateTaskId: 500 },
        names,
      ),
    ).toBe("Task 42 was created for Comp, which the plan did not create.");
    expect(
      describeDifference(
        { code: "claim_missing", taskId: 10, templateTaskId: 500 },
        names,
      ),
    ).toBe("comp is not linked to Comp.");
    expect(
      describeDifference(
        {
          code: "writeback_missing",
    ...
    expect(
      describeDifference({ code: "delete_missing", taskId: 10 }, names),
    ).toBe("comp was not deleted.");
    expect(
      describeDifference(
        { code: "unlink_missing", taskId: 10, templateTaskId: 500 },
        names,
      ),
    ).toBe("comp is still linked to Comp.");
    expect(
      describeDifference(
        { code: "edge_expected_missing", downstream: 10, upstream: 99 },
        names,
      ),
    ).toBe("The dependency comp on Task 99 was not added.");
    expect(
      describeDifference(
        {
    ...
    ).toBe("The kept dependency comp on Task 99 was not re-created.");
    expect(
      describeDifference({ code: "edge_still_present", edgeId: 3 }, names),
    ).toBe("Dependency 3 is still there.");
  });
});
...
describe("describeNote", () => {
  ...
  it("reads each undo note honestly", () => {
    expect(
      describeNote({ code: "edges_recreated", edgeIds: [3, 4] }, names),
    ).toBe(
      ...
      describeNote(
        {
          code: "dates_moved",
          taskId: 10,
          before: { start: "2026-11-01", due: "2026-11-02" },
          after: { start: "2026-11-03", due: null },
        },
        names,
      ),
    ).toBe(
      "comp: the apply moved its dates (2026-11-01 to 2026-11-02, now 2026-11-03 to none). Undo does not write them back: that would pin it.",
    );
    expect(
      describeNote(
        {
          code: "linked_twice_unmeasured",
          templateTask: 500,
          taskIds: [10, 11],
        },
        names,
    ...
    expect(describeNote({ code: "history_kept" }, names)).toBe(
      "The event log keeps the apply and the undo.",
    );
    expect(
      describeNote(
        {
          ...
          after: { start: "2026-11-03", due: "2026-11-04" },
        },
        names,
      ),
    ).toBe(
      "comp: the apply filled its dates from the template (now 2026-11-03 to 2026-11-04). Undo does not clear them: a null start date pins a Task with an upstream dependency.",
    );
    expect(
      describeNote({ code: "violation_may_change", taskIds: [10, 11] }, names),
    ).toBe(
...
describe("undoPreviewNotes", () => {
  it("warns before the undo: moved dates, erased edges that come back new, their downstream dates, twice-linked Tasks", () => {
    const e = (id: number, down: number, up: number) => ({
      id,
      downstream: down,
      upstream: up,
    ...
    expect(
      mergeNotes([
        ...
        [
          ...
          { code: "violation_may_change", taskIds: [5] },
          { code: "history_kept" },
        ],
  ...
  it("one row per entity: clean, with differences, failed with retry, undone, not applied", () => {
    ...
    const r = run([
      ...
      { entity: shot(6), status: { state: "done", undo: rec(6) } },
      {
        ...
        status: {
          state: "failed",
          error: { status: null, message: "Interrupted" },
          undo: rec(7),
        },
      },
    ]);
    const outcomes = [
      ...
      },
      {
        ...
        result: {
          ...
          createdFor: [],
          addedEdges: [],
          differences: [{ code: "delete_missing" as const, taskId: 10 }],
        },
      },
      {
        entity: shot(3),
