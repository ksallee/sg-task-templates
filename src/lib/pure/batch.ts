 ...
 *   1. claims: `template_task` on each claimed Task, conflict picks included;
 *   2. `task_template: null` when it already holds the template (084), then `task_template: T`;
 *   3. write-backs of kept field values, which the apply just overwrote (102);
 *   4. re-creates of kept edges the apply deleted (102), as TaskDependency rows (086, 095);
 *   5. extras: omit (status, which the apply keeps, 102) or delete (confirmed only).
 *
 ...
 * 102). `buildAfterApply` builds it from the post-apply read-back.
 *
...
const kept = (plan: EntityPlan, cause: AffectedEdge['cause']) =>
	plan.edges.affected.filter((a) => a.cause === cause && a.action === 'keep');

/**
 * Kept edges the apply deletes outright (non-template edges between linked Tasks, 102): re-created
 * after it, new id. `remove` sends nothing: the apply erases them. Replaced edges: `buildAfterApply`.
 */
export function edgeKeepRequests(plan: EntityPlan): BatchRequest[] {
	return kept(plan, 'not_in_template').map((a) => createEdge(a.existing));
}
...
export function extraRequests(plan: EntityPlan, opts: RunOptions, ctx: ProjectContext): BatchRequest[] {
	...
	for (const row of plan.rows) {
		...
		if (row.action === 'delete' && opts.deleteConfirmed) {
			// Retires the Task's TaskDependency rows too, revivable (089).
...
/**
 * Phase 2, from the read-back: date clearing on created Tasks (opt-in, 097), then replaced-edge
 * keeps (delete the template's copy, re-create the old edge: 085 allows one row per pair).
...
export function buildAfterApply(plan: EntityPlan, opts: RunOptions, after: AfterApply): BatchRequest[] {
	...
	for (const a of kept(plan, 'replaced')) {
 ...
 * one dependency per pair (085).
 */
export function validateRequests(reqs: BatchRequest[]): string[] {
	const errors: string[] = [];
