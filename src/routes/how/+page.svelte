...
<script lang="ts">
	...
	const fieldRows: Array<[string, string, string, string, string, string]> = [
		...
		['Milestone', 'ticked', 'unticked', 'ticked', 'ticked', 'not offered']
	];
	...
	<div class="mx-auto grid w-full max-w-5xl gap-10 px-4 pt-10 pb-20 sm:px-6 lg:grid-cols-[11rem_minmax(0,1fr)] lg:gap-14 lg:pt-14">
		...
		<article class="flex max-w-[42rem] min-w-0 flex-col gap-10 text-[0.9375rem] leading-relaxed">
			...
			<section class={section}>
				...
				<p>
					Names are compared loosely. Spaces at the ends are dropped, runs of spaces count as one, case is ignored.
				...
				<p>
					A Task linked to another template’s task, with the same name and step, is claimed for this one. The plan shows
					its old link. Undo puts it back.
				</p>
				...
				<p>
					The other becomes an extra. If one of the two is already linked to this template’s task, it wins outright: no
					conflict. A template with two tasks of the same name and step is a conflict too.
				</p>
			</section>

			<section class={section}>
				<h2 id="outcomes" class={h2}>{title('outcomes')}</h2>
				<p>The plan sorts every Task into one of five outcomes, per entity. In each picture, the entity’s Task is on the left, the template task on the right.</p>
				<dl class="bg-card flex flex-col divide-y rounded-lg border">
					{#each outcomes as [kind, meaning] (kind)}
						<div id={kind} class="flex scroll-mt-8 flex-col gap-2 p-4 sm:flex-row sm:gap-4">
							<dt class="flex w-40 shrink-0 items-center gap-3 self-start font-medium">
			...
			<section class={section}>
				...
				<p>
					Applying a template rewrites the Tasks it already holds, not only the new ones. On every kept or claimed Task,
					Flow PT overwrites a field when the template task has a value: name, step, estimate, description, sort order,
					reviewers, milestone and custom fields. Duration too, but only on a Task with no dates.
				</p>
				<p>
					An empty value on the template never clears a field. A 0 counts as a value: an estimate of 0 overwrites. An
					unticked milestone on the template leaves the Task’s alone.
				</p>
				...
				<figure class="flex flex-col gap-2">
					...
					<figcaption class="text-muted-foreground text-xs">What a kept or claimed Task ends with, per choice.</figcaption>
				</figure>
				<p>
					The name is the exception: it takes the template’s by default. Every rename shows in the plan, loudly when
					someone renamed the Task by hand.
				</p>
				<p>
			...
			<section class={section}>
				...
				<dl class="grid gap-3 sm:grid-cols-3">
					<div class="bg-card flex flex-col gap-1 rounded-lg border p-4">
						...
						<dd class="text-muted-foreground text-sm">Untouched.</dd>
					</div>
					<div class="bg-card flex flex-col gap-1 rounded-lg border p-4">
						<dt class="font-medium">Omit</dt>
						<dd class="text-muted-foreground text-sm">Its status is set to the project’s omit status. Undo puts the old one back.</dd>
					</div>
					<div class="bg-card border-destructive/40 flex flex-col gap-1 rounded-lg border p-4">
						<dt class="font-medium">Delete</dt>
						<dd class="text-muted-foreground text-sm">Asks twice. Undo revives the Task.</dd>
					</div>
				</dl>
				<p>
					A Task with Versions or published files can be deleted, with a loud warning that shows the counts: they stay,
					but lose their Task. Undo revives the Task and they point at it again.
				</p>
			</section>
			...
			<section class={section}>
				...
				<p>
					The apply adds every template dependency missing between the Tasks it links, whether they were kept, claimed
					or created.
				</p>
				...
				<p>
					...
					same type and offset. <span class="font-medium">Remove</span> lets it go. If re-creating one would close a loop,
				...
				<p>
					A new or re-created dependency moves the Task that waits on it at once, unless that Task is pinned. The plan
					lists the Tasks that may move. A pinned Task keeps its dates; the plan shows those that would be flagged with a
					dependency violation. Flow PT pins a Task that waits on another when its start date is set by hand.
				</p>
				<p>
					Created Tasks take the template’s dates, as the same calendar dates. On a created Task that waits on nothing,
			...
			<section class={section}>
				...
				<p>
					Each entity’s apply is sent as one request: it lands whole, or not at all. A failure stops that entity only;
					the result lists it with a retry.
				</p>
				<p>
					Every run keeps an undo record, per entity as it lands. It lives in this browser and downloads as a file; undo
					works from either. A tab closed mid-run offers the run again on reopen: continue it or undo it.
				</p>
				<div class="grid gap-3 sm:grid-cols-2">
					<div class="bg-card flex flex-col gap-3 rounded-lg border p-4">
						<h3 class="text-sm font-semibold">Undo puts back</h3>
						<ul class="flex flex-col gap-2 text-sm">
							{#each ['the entity’s old template, and the old links of claimed Tasks;', 'the fields as they were, names included, and assignees the apply filled;', 'the statuses of omitted Tasks;', 'deleted Tasks, revived with their Versions and published files;', 'dependencies, matched by their ends, type and offset.'] as line (line)}
						...
						<ul class="text-muted-foreground flex flex-col gap-2 text-sm">
							<li class="flex gap-2">
								<Minus class="mt-1 size-3.5 shrink-0" aria-hidden="true" /><span
									...
									Task that had none are cleared only where nothing runs into the Task; elsewhere they stay, and the result
									says so.</span
								>
							</li>
							<li class="flex gap-2">
								<Minus class="mt-1 size-3.5 shrink-0" aria-hidden="true" />Some dependencies come back as new ones: same ends,
								type and offset, a new id.
							</li>
			...
			<section class={section}>
				...
				<p>
					The app writes as you, signed in through Flow PT’s App Session Launcher. Every change is in the event log under
				...
				<p>
					...
					an update, a create and a delete that Flow PT refuses before anything lands. Nothing is written. If you look
					short of what the plan needs, it warns.
				</p>
				<p>
					It’s a hint, not a promise: measured for a scripted user acting as an Artist, not yet for a person signed in. A
					write refused during the apply is reported per entity.
				</p>
			</section>
